...
import { Sparkles, ArrowLeft, Trophy, LogOut, Map, Clock, BookOpen, LogIn } from "lucide-react"
      ...
      <div className="min-h-screen bg-transparent p-4 md:p-8">
        <div className="mx-auto max-w-4xl relative z-10">
          ...
          <div className="mb-8 text-center">
            <div className="mb-2 text-6xl animate-bounce-gentle emoji-icon">{subject?.icon}</div>
            ...
            {levels.map((level) => (
              ...
              >
                <div className="p-6 text-center">
                  <div className="text-5xl mb-3 animate-float emoji-icon">{level.emoji}</div>
                  <div
                  ...
                  >
                    <span className="emoji-icon">{level.icon}</span>
                  </div>
    ...
    <div className="min-h-screen bg-transparent p-4 md:p-8">
      <div className="mx-auto max-w-6xl relative z-10">
        <div className="flex justify-between items-center mb-8 gap-4 flex-wrap">
          <div className="flex flex-wrap gap-2">
            <Link href="/leaderboard">
              <Button className="kid-btn bg-gradient-to-r from-amber-500 to-orange-600 text-white flex items-center gap-1.5 px-3 py-1.5 sm:py-2 text-xs sm:text-sm md:text-base">
                <Trophy className="h-4 w-4 sm:h-5 sm:w-5 shrink-0" />
                <span className="hidden sm:inline">Leaderboard</span>
              </Button>
            </Link>
            <Link href="/history">
              <Button className="kid-btn bg-gradient-to-r from-blue-500 to-cyan-600 text-white flex items-center gap-1.5 px-3 py-1.5 sm:py-2 text-xs sm:text-sm md:text-base">
                <Clock className="h-4 w-4 sm:h-5 sm:w-5 shrink-0" />
                <span className="hidden sm:inline">My Progress</span>
              </Button>
            </Link>
            <Link href="/explore-map">
              <Button className="kid-btn bg-gradient-to-r from-cyan-500 to-teal-600 text-white flex items-center gap-1.5 px-3 py-1.5 sm:py-2 text-xs sm:text-sm md:text-base">
                <Map className="h-4 w-4 sm:h-5 sm:w-5 shrink-0" />
                <span className="hidden sm:inline">Explore Map</span>
              </Button>
            </Link>
            <Link href="/practice">
              <Button className="relative kid-btn bg-gradient-to-r from-emerald-500 to-teal-600 text-white flex items-center gap-1.5 px-3 py-1.5 sm:py-2 text-xs sm:text-sm md:text-base">
          ...
          <div className="flex gap-2 items-center flex-wrap">
            {session ? (
              <>
                <div className="text-xs sm:text-sm text-muted-foreground mr-1 bg-white/70 backdrop-blur-sm rounded-full px-3 py-1.5 border border-sky-100 font-medium">👋 {profile?.name || session?.user?.name || session?.user?.email || "Student"}</div>
                <Button onClick={handleLogout} className="kid-btn bg-gradient-to-r from-red-500 to-rose-600 text-white flex items-center gap-1.5 px-3 py-1.5 sm:py-2 text-xs sm:text-sm md:text-base">
              ...
              <>
                ...
                <Link href="/auth/sign-up">
                  <Button className="kid-btn bg-gradient-to-r from-green-500 to-emerald-600 text-white flex items-center gap-1.5 px-3 py-1.5 sm:py-2 text-xs sm:text-sm md:text-base">
        ...
        <div className="mb-12 text-center relative">
          {/* Dodo Mascot - positioned to the right to avoid button overlap, visible on lg and up */}
          <div className="absolute -right-2 lg:right-0 xl:-right-4 top-8 hidden lg:block z-20 pointer-events-none">
            <div className="pointer-events-auto">
              <DodoMascot mood="idle" size="lg" showSpeechBubble speechText={welcomeMessage} speechPlacement="bottom" />
            </div>
          </div>
          
          {/* Fun island illustration */}
          <div className="text-7xl sm:text-8xl mb-4 animate-bounce-gentle emoji-icon">🏝️</div>
          
          <div className="mb-4 flex items-center justify-center gap-3">
            <span className="animate-float flex items-center">
              <DodoIcon size={40} className="drop-shadow-md" />
            </span>
            ...
            <span className="text-3xl animate-float emoji-icon" style={{ animationDelay: "0.5s" }}>🌺</span>
          </div>
          <p className="text-lg sm:text-xl md:text-2xl text-muted-foreground mt-4">
            Choose your subject and start the adventure! <span className="emoji-icon">🎓🌴</span>
          </p>
        </div>
            ...
            >
              <div className="p-4 sm:p-6 md:p-8 text-center">
                ...
                <div className="flex justify-center gap-3 mb-3">
                  <span className="text-2xl animate-float emoji-icon">{subject.emoji2}</span>
                  <span className="text-2xl animate-float emoji-icon" style={{ animationDelay: "0.4s" }}>✨</span>
                </div>
                <div
        ...
        <Card className="kid-card border-accent/40 bg-gradient-to-r from-amber-50/80 via-sky-50/80 to-emerald-50/80">
          <div className="p-8 text-center">
            <div className="flex justify-center items-center gap-3 mb-4">
              <DodoIcon size={36} className="drop-shadow-sm" />
              <span className="text-3xl emoji-icon">🌴</span>
              <span className="text-3xl emoji-icon">🏝️</span>
            </div>
            <h3 className="mb-3 text-2xl font-bold text-card-foreground kid-heading">Welcome to the Adventure! <span className="emoji-icon">🌴</span></h3>
